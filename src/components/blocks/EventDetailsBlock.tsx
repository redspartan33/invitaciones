  ...
  return (
    <BlockWrapper style={block.style}>
      <div className="flex flex-col items-center gap-4 text-center">
        <EventIcon kind={data.icon} className="h-10 w-10" />
        <div>
          <p className="font-serif text-2xl">{formatDate(data.date, 'DD MMMM YYYY')}</p>
          <p className="mt-1 text-sm uppercase tracking-widest text-ink-500">{data.time}</p>
        </div>
        <div>
          <p className="text-lg font-medium">{data.location}</p>
          {data.address && <p className="text-sm text-ink-500">{data.address}</p>}
        </div>
        {data.description && <p className="max-w-md text-sm text-ink-600">{data.description}</p>}
      </div>
    </BlockWrapper>
  )
}
