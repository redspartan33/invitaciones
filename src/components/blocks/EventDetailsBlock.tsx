  ...
  return (
    <BlockWrapper style={block.style}>
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        {(data.date || data.time) && (
          <div>
            {data.date && <p className="font-serif text-2xl">{formatDate(data.date, 'DD MMMM YYYY')}</p>}
            {data.time && <p className="mt-1 text-sm uppercase tracking-widest opacity-70">{data.time}</p>}
          </div>
        )}
        {(data.location || data.address) && (
          <div>
            {data.location && <p className="text-lg font-medium">{data.location}</p>}
