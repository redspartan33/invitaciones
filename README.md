...
- **Banners / Promos** (`promoBanner`) — Toggle que al activarse permite editar un carrusel con cards personalizables. Cada slide lleva imagen opcional, título, subtítulo, botón con enlace y colores de fondo/texto. Controles globales: forma (16:9, 3:1, 1:1), intervalo en segundos, autoplay on/off, indicadores on/off. El carrusel renderiza entre el `menu-header` y la primera `menu-section` en la vista pública (y en la canvas del editor). Swipe táctil nativo, flechas y dots, autoplay pausado en hover. CTAs con `data-track="promo-cta"` para que aparezcan en el dashboard de métricas.
