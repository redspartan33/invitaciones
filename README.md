...
- **Animación de apertura** — estilo de la transición cuando el sobre termina de abrir y entrega la invitación, elegible entre: **Clásica (solapa)** (ahora con un sutil zoom + blur de salida), **Zoom / estallido** (con leve giro), **Cortina** (clip-path que cierra al centro), **Disolución (blur)** (blur + brightness), **Destellos ✨**, **Pétalos cayendo 🌸** y **Confeti 🎉** — las tres últimas con capas de partículas más densas y con coreografía más elaborada (vaivén, giro y estela). Guardada en `envelopeIntro.openAnimation` (default `'classic'`). La coreografía de solapa + tarjeta se comparte; sólo cambian la transición de salida del overlay (`OVERLAY_EXIT`) y las capas decorativas (`SparkleLayer` / `PetalsLayer` / `ConfettiLayer`).
- **Partículas personalizables** — para las animaciones con partículas (Destellos / Pétalos / Confeti) se puede definir una **imagen o SVG propio** (`envelopeIntro.particleImage`) que reemplaza la forma predeterminada; si se deja vacía, se usa la forma incorporada con un **color de partícula** configurable (`envelopeIntro.particleColor`). Ambos campos se renderizan en un panel "Partículas" que sólo aparece al elegir una animación de partículas.
