...
15. **Flotante** — Elemento con **imagen y/o texto** (ambos opcionales) que se posiciona **encima** de la invitación, no en el flujo apilado. Dos modos por bloque (`anchorMode`): **"Libre en el lienzo"** (se arrastra a cualquier parte sobre el canvas, baja con el scroll) o **"Fijo en pantalla"** (queda pegado a una esquina, visible mientras el invitado hace scroll). Solo invitaciones. Ver [FloatingBlock](src/components/blocks/FloatingBlock.tsx), la capa editable [FloatingLayer](src/components/editor/FloatingLayer.tsx) y los helpers de posición [floatingLayout.ts](src/utils/floatingLayout.ts). En el editor se arrastra directamente sobre el canvas; en la vista pública los flotantes se separan del flujo (`flowVisible` vs `floatingBlocks` en [PublicInvitationView](src/components/public/PublicInvitationView.tsx)). **Layout independiente por vista (mobile/desktop):** cada flotante libre guarda posición/tamaño/rotación por separado en `data.views.{mobile,desktop}` (tipo `FloatViewLayout`); el selector de viewport del editor decide qué breakpoint se edita (mobile vs desktop; tablet comparte desktop) y la vista pública elige el layout real con el hook `useFloatView` (media query `max-width:768px`). Mientras una vista no se edita, **hereda** de la otra re-escalada a su ancho de columna de referencia (390 ↔ 920 px) para que un diseño hecho en mobile **no se agrande** en desktop; en cuanto la tocas, ese breakpoint queda congelado e independiente (`resolveFloatLayout` / `refit` en [floatingLayout.ts](src/utils/floatingLayout.ts), acción `updateFloatView` en el store). Los flotantes **libres en el lienzo** muestran, al seleccionarlos, un **contorno de selección azul** (no negro) con **4 tiradores de esquina para estirar el ancho**, un **tirador superior para girar** y además **4 zonas de giro en las esquinas** (al acercarte a una esquina puedes rotar; el punto exacto de la esquina estira, el aro alrededor gira), todo con **imán a 0/45/90/…°** directamente sobre el canvas — sin campos numéricos. La matemática proyecta el arrastre sobre el eje propio del elemento, así que estirar funciona aun con el bloque rotado. Pueden **salirse libremente del marco** (el editor permite un amplio overflow y la vista pública usa `overflow-x-clip` en la raíz, así que sangran fuera del borde **sin generar scroll horizontal**). Además aceptan una **sombra opcional** (`data.shadow`: `soft`/`medium`/`strong` o `custom`) renderizada como `drop-shadow()` para que siga el contorno alpha de los stickers PNG/SVG; el modo **Avanzada** expone color, desenfoque, desplazamiento X/Y y opacidad (helper `floatingShadowFilter` en [floatingLayout.ts](src/utils/floatingLayout.ts), sección **Sombra** en [DynamicBlockForm](src/components/forms/DynamicBlockForm.tsx)).
...
- **Formulario de confirmación** — Toggle "Usar formulario en vez de WhatsApp". Al activarlo se genera automáticamente un link público único (`/?guestlist=<slug>`) atado a la invitación y se inicializa el archivo en Vercel Blob. El botón "Confirmar asistencia" abre el formulario (nombre + mensaje opcional) **en un modal** centrado sobre un backdrop oscuro, en lugar de empujar el cuerpo de la invitación; se cierra con la ✕, el botón Cancelar, click fuera o tecla Escape. El modal se renderiza con `createPortal` a `document.body` y `z-[120]` (mismo patrón que [ImageLightbox](src/components/blocks/ImageLightbox.tsx)) para que quede **por encima de los bloques flotantes** anclados a pantalla (que usan `z-50` y crearían un atrapamiento de stacking context si el modal se montara dentro del árbol del bloque). Al enviar (éxito o error) el modal se cierra y se muestra el feedback inline correspondiente. La sección **Formulario** del config panel permite personalizar el **título** y el **placeholder** del campo Mensaje; el mismo título se usa como etiqueta sobre cada mensaje en la página de confirmados (resuelto vía `GET /api/guestlists/<slug>/meta`, que busca el bloque RSVP cuyo `guestListSlug` coincide).
...
- **Imágenes en platillos** (`enableItemImages`) — Cuando está prendido, los platillos con `image` se renderizan con miniatura (80×80, `object-cover`, esquinas redondeadas) a la izquierda del nombre/descripción/precio. Los platillos sin imagen mantienen el layout text-only. La imagen se sube desde el form de cada platillo en [MenuItemsForm](src/components/forms/MenuItemsForm.tsx) (mismo patrón que ImageSet: URL o `FileReader` con cap de 2 MB). Las `data:` URIs se suben a `/api/assets` automáticamente al publicar (`IMAGE_KEYS` ya incluye `'image'` y el walker recorre los arrays). La miniatura es un botón: al tocarla/clic abre la foto en grande dentro de un lightbox a pantalla completa ([ImageLightbox](src/components/blocks/ImageLightbox.tsx), vía `createPortal` sobre `document.body`) que cierra con el backdrop, el botón × o la tecla Escape y bloquea el scroll del fondo mientras está abierto. Lleva un ícono ⤢ sobre la miniatura como pista de que se puede ampliar.
...
- **Efecto de partículas** — capa decorativa que cae/flota durante la apertura, **independiente** de la transición de salida. Elegible entre **Ninguno**, **Destellos ✨**, **Pétalos 🌸**, **Confeti 🎉**, **Lluvia 🌧️**, **Hojas al viento 🍃**, **Bokeh 🔮**, **Luciérnagas 🐝**, **Burbujas 🫧**, **Corazones 💗**, **Nieve ❄️**, **Estrellas fugaces 🌠** y **Brasas 🔥**. Cada efecto es un layer con su seed determinista (no se rebaraja entre renders) y un fallback CSS/SVG. Guardado en `envelopeIntro.particleEffect` (default `'none'`). El despachador `ParticleLayer` mapea el efecto a su componente.
- **Imágenes de partícula (al azar)** — se puede definir una **lista de imágenes/SVG** (`envelopeIntro.particleImages`, por URL o subiendo archivo con cap de 2 MB) que reemplaza la forma predeterminada; **cada partícula elige una al azar** de forma determinista (`Particle` con `seed`). Si la lista está vacía se usa la forma incorporada con un **color** configurable (`envelopeIntro.particleColor`). (El campo legacy `particleImage` se sigue leyendo como fallback de una sola imagen.)
- **Transición de entrada** — selector **independiente** de cómo el overlay entrega la invitación, con controles: **Estilo** (`envelopeIntro.transition`: **Fundido**, **Zoom**, **Iris** clip circular, **Cortina**, **Deslizar hacia arriba**, **Disolución**), **Duración** (`transitionDuration`, slider 0.3–1.5 s) y **Tiempo en pantalla** (`holdDuration`, slider 1–6 s, cuánto se mantiene la tarjeta antes de salir). Para que el hand-off sea **fluido**, el overlay se anima **durante la etapa `leaving`** (revelando la página debajo) en vez de hacer un corte seco al desmontar, y la coreografía de la tarjeta/sobre se sincroniza con la duración elegida (`overlayExit` + `EnvelopeStage`). El antiguo `openAnimation` (combinado) se sigue leyendo y se mapea a `(particleEffect, transition)` vía `resolveLegacy` para invitaciones existentes.
